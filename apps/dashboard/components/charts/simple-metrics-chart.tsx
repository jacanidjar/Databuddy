...
export function SimpleMetricsChart({
	...
	const hasVariation = hasData && metrics.some((m) => {
		const values = data.map((d) => d[m.key]).filter((v) => v != null) as number[];
		return values.length > 1 && values.some((v) => v !== values[0]);
	});

	return (
		<Card className={cn("group gap-0 overflow-hidden border bg-card py-0 transition-colors hover:border-primary", className)}>
			...
			<div className="dotted-bg bg-accent">
				{!hasData ? (
					<div className="flex flex-col items-center justify-center" style={{ height }}>
						...
						<span className="mt-2 text-[10px] text-muted-foreground/60">No data</span>
					</div>
				) : !hasVariation ? (
						...
						<AreaChart data={data} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
							...
							<Tooltip
								content={({ active, payload, label }) => (
									<ChartTooltip
										active={active}
										entries={createTooltipEntries(payload as Array<{ dataKey: string; value: number; color: string }>, metricsWithColors)}
			...
			<div className="flex items-center gap-2.5 border-t px-2.5 py-2.5">
				<div className="min-w-0 flex-1">
					{title && (
						<p className="truncate font-semibold text-sm leading-tight">{title}</p>
					)}
					{description && (
