...
<CodeBlock
  ...
  code={`<Databuddy
    ...
    trackPerformance // Track loading times
    trackWebVitals // Track Core Web Vitals
    trackErrors // Track JavaScript errors
    // Optional tracking (enable as needed)
    trackOutgoingLinks // Track external link clicks
    trackScrollDepth // Track scroll behavior
    trackInteractions // Track button clicks and form submissions
    trackAttributes // Track elements with data-track attributes
  />`}
/>
