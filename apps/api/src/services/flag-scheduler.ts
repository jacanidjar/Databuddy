...
export async function executeSchedule(sched: ExecutableSchedule) {
	try {
		...
		if (!flag) {
			logger.warn(
				{ scheduleId: sched.id, flagId: sched.flagId },
				"Flag not found for schedule, deleting schedule"
			);
		...
		if (sched.__isStep && sched.rolloutSteps) {
			...
			const allStepsExecuted = updatedRolloutSteps.every(
				(step) => step.executedAt
			);
			...
			await db
				.update(flagSchedules)
				.set({ isEnabled: false, executedAt: new Date() })
				.where(eq(flagSchedules.id, sched.id));
		}
		...
		if (updatedFlag) {
			...
			logger.warn(
				{ flagId: sched.flagId, scheduleId: sched.id },
				"Failed to update flag, schedule not executed"
			);
		}
