...
const webhookBodySchema = z.object({
	...
	stepValue: z
		.union([z.number(), z.literal("enable"), z.literal("disable")])
		.optional(),
});
export const flagSchedulerWebhook = new Elysia({
	prefix: "/webhooks/flag-scheduler",
})
	.post("/", async function handleFlagSchedulerWebhook({ request, set }) {
		try {
			...
			const scheduleId =
				body.scheduleId || request.headers.get("X-Schedule-Id");
			if (!scheduleId) {
			...
			if (
				...
				body.stepScheduledAt &&
				body.stepValue !== undefined
			) {
				...
				if (stepAlreadyExecuted) {
					logger.info(
						{ scheduleId, stepScheduledAt: body.stepScheduledAt },
