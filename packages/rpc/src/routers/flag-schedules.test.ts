...
describe("flagSchedulesRouter", () => {
	...
	describe("create", () => {
		it("creates a schedule successfully", async () => {
			mockDb.query.flags.findFirst.mockResolvedValue({
				id: "flag-1",
				websiteId: "site-1",
			});
			const mockReturn = [{ id: "sched-1", flagId: "flag-1" }];
			const valuesMock = mock(() => ({
				returning: mock(() => Promise.resolve(mockReturn)),
			}));
			...
			expect(mockAuthorizeWebsiteAccess).toHaveBeenCalledWith(
				mockContext,
				"site-1",
				"update"
			);
		...
		it("throws if flag not found", async () => {
			...
			const handler = (flagSchedulesRouter.create as any)["~orpc"].handler;
			expect(handler({ context: mockContext, input })).rejects.toThrow(
				"Flag not found"
			);
		});
	});
	...
	describe("update", () => {
		it("updates a schedule successfully", async () => {
			...
			mockDb.query.flagSchedules.findFirst.mockResolvedValue({
				id: "sched-1",
				flagId: "flag-1",
			});
